...
func (cq *CommentQuery) QuerySkus() *SkuQuery {
	query := (&SkuClient{config: cq.config}).Query()
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withSkus; query != nil {
		if err := cq.loadSkus(ctx, query, nodes, nil,
			func(n *Comment, e *Sku) { n.Edges.Skus = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadSkus(ctx context.Context, query *SkuQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Sku)) error {
...
func (cq *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withSkus != nil {
			_spec.Node.AddColumnOnce(comment.FieldSkuID)
		}
	}
