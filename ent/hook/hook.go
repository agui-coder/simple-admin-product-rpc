...
func (f PropertyValueFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PropertyValueMutation); ok {
		return f(ctx, mv)
	}
...
type SpuFunc func(context.Context, *ent.SpuMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SpuFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SpuMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SpuMutation", m)
}
