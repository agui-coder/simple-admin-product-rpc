...
type PropertyQuery struct {
	...
	withPropertyValues *PropertyValueQuery
...
func (pq *PropertyQuery) QueryPropertyValues() *PropertyValueQuery {
	query := (&PropertyValueClient{config: pq.config}).Query()
...
func (pq *PropertyQuery) Clone() *PropertyQuery {
	...
	return &PropertyQuery{
		...
		withPropertyValues: pq.withPropertyValues.Clone(),
...
// WithPropertyValues tells the query-builder to eager-load the nodes that are connected to
// the "property_values" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PropertyQuery) WithPropertyValues(opts ...func(*PropertyValueQuery)) *PropertyQuery {
	query := (&PropertyValueClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPropertyValues = query
	return pq
}
...
func (pq *PropertyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Property, error) {
	...
	if query := pq.withPropertyValues; query != nil {
		if err := pq.loadPropertyValues(ctx, query, nodes,
			func(n *Property) { n.Edges.PropertyValues = []*PropertyValue{} },
			func(n *Property, e *PropertyValue) { n.Edges.PropertyValues = append(n.Edges.PropertyValues, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PropertyQuery) loadPropertyValues(ctx context.Context, query *PropertyValueQuery, nodes []*Property, init func(*Property), assign func(*Property, *PropertyValue)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Property)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(propertyvalue.FieldPropertyID)
	}
	query.Where(predicate.PropertyValue(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(property.PropertyValuesColumn), fks...))
	}))
...
func (pq *PropertyQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(property.Table, property.Columns, sqlgraph.NewFieldSpec(property.FieldID, field.TypeUint64))
	_spec.From = pq.sql
