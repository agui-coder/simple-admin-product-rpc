...
func (bq *BrandQuery) QuerySpus() *SpuQuery {
	query := (&SpuClient{config: bq.config}).Query()
...
func (bq *BrandQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Brand, error) {
	...
	if query := bq.withSpus; query != nil {
		if err := bq.loadSpus(ctx, query, nodes,
			func(n *Brand) { n.Edges.Spus = []*Spu{} },
			func(n *Brand, e *Spu) { n.Edges.Spus = append(n.Edges.Spus, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BrandQuery) loadSpus(ctx context.Context, query *SpuQuery, nodes []*Brand, init func(*Brand), assign func(*Brand, *Spu)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(spu.FieldBrandID)
	}
	query.Where(predicate.Spu(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(brand.SpusColumn), fks...))
	}))
...
func (bq *BrandQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(brand.Table, brand.Columns, sqlgraph.NewFieldSpec(brand.FieldID, field.TypeUint64))
	_spec.From = bq.sql
